...
| Bridge + cargo | `--rust-bridge --rust-cargo` | Cross-checks the approved checker with Cargo locked/offline |
...
the approved Ghoti binary is invoked as a locked, offline local cross-check
(`cargo run --locked --offline -- --input <tmp>`). Arbitrary Cargo manifests
are refused, and compiler artifacts are directed to a temporary target
directory. The binary's own safety block reports
...
(23 tests) proves, for dry-run plans:
...
| `01_projects/runtime_mvp/tests/test_n6_33a_rust_policy_bridge_computer_use.py` | 23 tests |
...
- The optional `cargo` cross-check is local, locked, offline, and restricted to
  Ghoti's committed policy-checker manifest. It neither installs nor launches
  anything; compiler artifacts stay outside the repo.
