...
The guard does **not** rewrite history and does **not** force-push. It prevents
future contamination and adds checks. Already-pushed inherited commits are left
as-is and surfaced as a reviewed warning (see "No-rewrite policy" below).
...
## No-rewrite policy (why history is left alone)

The three in-flight feature branches (N+6.29A, N+6.30A, N+6.31A) contain commits
that were made under the old AI identity. Per mission hard rules:

- **No force-push.**
- **No history rewrite.**

So those commits are not altered. Their inherited authorship is reported by
`public_repo_security_audit.py` as a reviewed **warning**, not a blocker. The
guard ensures every **new** commit (including this one) uses the human operator
identity and carries no attribution trailer. When a clean human-authored commit
is added on top of each branch, that branch's HEAD attribution check passes.
