...
2. **Engine selection** — `_select_engine()` maps `task_type` to preferred engine; falls back to lowest trial_order among MOST_READY / SECOND_READY / CLI_ONLY. Engines with globally-blocked capabilities are never selected.
...
| `01_projects/runtime_mvp/tests/test_n6_35a_plug_and_play_agent_systems_trial.py` | 52 tests |
