...
The only allowlisted worker is `repo-summary-worker`. It reads declared
repo-local context files and returns a small source-linked summary. It cannot
...
6. The runner starts exactly one fixed Python entrypoint with a fixed
   `--json` argument and sanitized environment.
...
9. The runner writes the artifact, run record, evidence note, handoff, and
   final runner state.
...
- `worker_id` is exactly `repo-summary-worker`;
- task is allowlisted;
...
Future workers must be added individually to both the Rust and Python
