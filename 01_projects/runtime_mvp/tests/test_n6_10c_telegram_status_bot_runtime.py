...
# The real chat id the operator mentioned externally must never be committed.
FORBIDDEN_CHAT_ID = "8825693267"
...
class SecretAndPrivacyTests(unittest.TestCase):
    def test_no_real_chat_id_committed(self):
        for path in PACK_TEXT_FILES:
            self.assertNotIn(FORBIDDEN_CHAT_ID, read(path), msg=f"chat id leaked in {path.name}")
