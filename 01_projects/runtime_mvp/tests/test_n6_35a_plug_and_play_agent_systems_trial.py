...
class TestInventory(unittest.TestCase):
    ...
    def test_verified_source_urls(self):
        inv = {repo["id"]: repo for repo in _load_inventory()}
        expected = {
            "claude_swarm": "https://github.com/affaan-m/claude-swarm",
            "am_will_swarms": "https://github.com/am-will/swarms",
            "clawteam": "https://github.com/HKUDS/ClawTeam",
            "ruflo": "https://github.com/ruvnet/ruflo",
            "ecc": "https://github.com/affaan-m/ecc",
            "paperclip": "https://github.com/paperclipai/paperclip",
            "hermes_paperclip_adapter": "https://github.com/nousresearch/hermes-paperclip-adapter",
        }
        self.assertEqual({repo_id: inv[repo_id]["source_url"] for repo_id in expected}, expected)
...
class TestGloballyBlockedPreFilter(unittest.TestCase):
    ...
    def test_task_requested_live_capabilities_are_refused(self):
        for capability in ("live_launch", "auto_submit", "account_actions", "browser", "mcp"):
            with self.subTest(capability=capability):
                result = self._run(
                    _task_spec("swarm_coordination", capabilities=[capability])
                )
                self.assertFalse(result["ok"])
                self.assertEqual(result["error"], "globally_blocked_request")
                self.assertIn(capability, result["blocked_capabilities"])

    def test_task_requested_live_action_is_refused(self):
        result = self._run(
            _task_spec("swarm_coordination", actions=["live_launch"])
        )
        self.assertFalse(result["ok"])
        self.assertEqual(result["error"], "globally_blocked_request")


class TestDualGateFailClosed(unittest.TestCase):
    def test_missing_adapter_fails_closed(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            original = _planner._repo_root
            try:
                _planner._repo_root = lambda: Path(temp_dir)
                result = _planner._validate_plan(plan)
            finally:
                _planner._repo_root = original
