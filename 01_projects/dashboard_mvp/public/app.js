...
function agentOsRenderRunner(payload) {
  ...
  const latest_local_model_state = payload.latest_local_model_state;
  out.innerHTML = '<p><strong>Allowlist:</strong> '
    ...
    + '<p><strong>Latest local-model summary/classification:</strong> '
    + (latest_local_model_state
      ? '<code>' + escapeHtml(latest_local_model_state.status || "?") + '</code> '
        + escapeHtml(latest_local_model_state.artifact_path || latest_local_model_state.request_id || "")
      : 'none') + '</p>'
...
async function agentOsFullLocalModelWorkerDemo() {
  const label = document.getElementById("agentos-runner-action-status");
  if (label) label.textContent = "Running approved local summary/classification worker...";
  try {
    const payload = await requestJson(
      "/api/product-control/agent-os-full-local-model-worker-demo",
      { method: "POST", body: JSON.stringify({}) },
    );
    if (label) label.textContent = payload.ok
      ? "Summary/classification complete: " + (payload.artifact_path || "?")
      : "Summary/classification demo failed: " + (payload.reason || payload.failed_step || "?");
  } catch (err) {
    if (label) label.textContent = "Summary/classification demo failed: " + err.message;
  }
  refreshAgentOsApprovals();
  refreshAgentOsRunner();
  refreshAgentOsLatest();
...
function initAgentOs() {
  ...
  document.getElementById("agentos-full-local-model-worker-demo")?.addEventListener(
    "click", agentOsFullLocalModelWorkerDemo);
  refreshAgentOsStatus();
