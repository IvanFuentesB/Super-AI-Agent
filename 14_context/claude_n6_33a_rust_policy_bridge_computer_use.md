...
- `_invoke_rust_policy_checker()` is an optional, locked/offline `cargo`
  cross-check (`--rust-cargo`) restricted to Ghoti's committed policy-checker
  manifest; it degrades safely to `available: false` if Rust is absent.
...
- `03_scripts/computer_use_adapter/ghoti_computer_use_adapter.py` — opt-in bridge
  (mapping, mirror, approved-manifest-only cargo cross-check, CLI flags
  `--rust-bridge`, `--rust-cargo`, `--rust-manifest`).
...
AI attribution. Feature branch only. The cargo cross-check is local, locked,
offline, and restricted to the committed Ghoti policy-checker manifest.
