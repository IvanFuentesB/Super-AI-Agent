...
- N+6.28B (Rust policy checker) is now merged and active — bridge wiring to
  `ghoti_computer_use_adapter.py` is the next concrete step after this intake.
- N+6.27B (swarm_launcher) is merged — dry-run only; live launch remains off.
- N+6.29B (computer_use_adapter) is merged; those files remain untouched here.
...
1. **Next runtime lane:** wire the Rust policy bridge in `ghoti_computer_use_adapter.py`
2. **Keep computer-use dry-run only** until a separately audited milestone
