...
| N+6.29A | Repo-backed adapter contract; URL guard; Arena payload | not merged (N+6.29B pending) |

**N+6.29A must merge before any further computer-use milestone adds files in
`03_scripts/computer_use_adapter/` or `14_context/computer_use_adapter/`.**
...
N+6.30B  → wire Rust policy checker bridge (post N+6.29B merge)
...
## Next Steps After N+6.29B Merge

1. **N+6.29B merges** → `computer_use_adapter` files available on main.
2. **Wire Rust bridge** — extend `ghoti_computer_use_adapter.py --plan` to pipe
