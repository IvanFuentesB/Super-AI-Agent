...
**Base:** current `origin/main` includes N+6.35B during the Codex audit merge
**Status:** AUDIT HARDENED — external CLI remains blocked
...
- Has `--dry-run` flag, but source inspection confirms it requires an API key, calls the decomposition model, and writes a session; external invocation remains blocked.
...
- Audit hardening: dual-gate failures deny by default; sandbox overrides outside the approved runtime sandbox are refused.
- Audit hardening: status output uses repo-relative paths and refuses detected targets that resolve outside the approved sandbox.
- Audit hardening: smoke acceptance requires explicit N+6.33A bridge readiness and an allowing Rust-policy decision.

## What was actually run

- File reads: `pyproject.toml`, `README.md`, `LICENSE` from `claude_swarm` sandbox repo
- No third-party code imported or executed
- Ghoti-native adapter smoke: no network calls, no API keys, no real agents
...
Audit the external command's provider/API and session-write behavior, then use
a synthetic fixture through the dual gate. Human approval is required before
