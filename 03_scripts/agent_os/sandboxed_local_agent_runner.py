...
WORKER_REGISTRY = {
    "repo-summary-worker": Path(__file__).resolve().parent / "repo_summary_worker.py",
}
...
def execute_allowlisted_worker_request(
    ...
    *,
    ...
    try:
        ...
        worker_path = WORKER_REGISTRY[payload["worker_id"]].resolve()
    ...
    try:
        process = subprocess.Popen(
            [sys.executable, str(worker_path), "--json"],
            cwd=str(repo_root),
        ...
        stdout, _stderr = process.communicate(timeout=TERMINATE_GRACE_SECONDS)
