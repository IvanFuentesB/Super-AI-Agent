...
def build_local_worker_request(
    *,
    ...
    """Build one deterministic request for an allowlisted local worker."""
    local_model = workflow_id == "local-model-summary-classification"
    worker_id = (
        "local-model-summary-classification-worker"
        if local_model
        else "repo-summary-worker"
    )
    selected_task = task or ("summary_classification" if local_model else "repo_status_summary")
    created = created_at or _now()
    seed = {
        "workflow_id": workflow_id,
    ...
    return {
        ...
        "max_runtime_seconds": max_runtime_seconds,
        "approval_token_hash": None,
        "summary": f"Run one approved {worker_id} for {workflow_id}.",
        "risk_note": (
