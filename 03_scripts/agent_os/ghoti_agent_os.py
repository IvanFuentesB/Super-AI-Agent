...
def cmd_propose_worker_run(workflow_id: str) -> dict:
    ...
    if workflow_id == "local-model-summary-classification":
        ...
        result["worker_id"] = request["payload"]["worker_id"]
        result["suggestion_path"] = None
        result["safety_flags"] = _safety_flags()
        return result
    template = workflow_templates.get_template(workflow_id)
    if template is None:
...
def cmd_full_local_model_worker_demo() -> dict:
    """Run the deterministic summary/classification worker through the full gate."""
    proposed = cmd_propose_worker_run("local-model-summary-classification")
    if not proposed.get("ok"):
        return {
            "ok": False,
            "action": "full-local-model-worker-demo",
            "failed_step": "propose",
            "result": proposed,
            "safety_flags": _safety_flags(),
        }
    ...
    return {
        **executed,
        ...
        "explicit_approval_recorded": True,
        "rust_guard_validated": bool((executed.get("guard_decision") or {}).get("allow")),
        "provider_api_used": bool(executed.get("provider_api_used")),
        "model_mode": executed.get("model_mode"),
        "safety_flags": {
            **_safety_flags(),
            "agents_launched": bool(executed.get("local_agent_process_executed")),
        },
...
def cmd_check() -> dict:
    ...
    record(
        "sandboxed_runner_has_two_allowlisted_workers",
        runner_status.get("worker_allowlist")
        == [
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--full-local-model-worker-demo", action="store_true")
    ...
    elif args.full_local_model_worker_demo:
        payload = cmd_full_local_model_worker_demo()
    else:
        parser.print_help()
        return 2
